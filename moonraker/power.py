        ...
        try:
            for section in prefix_sections:
                ...
                if dev_type == "gpio":
                    ...
                    dev = HomeSeer(cfg)
                elif dev_type == "shell_command":
                    dev = ShellCommand(cfg)
                else:
...
class ShellCommand(PowerDevice):
    def __init__(self, config):
        super().__init__(config)
        self.on = config.get("on")  # 开机命令
        self.off = config.get("off")  # 关机命令
    ...
    def set_power(self, state):
        # state == "on" 就打开电源，state == "off" 就关闭电源
        if state == "on":
            result = subprocess.Popen(self.on, shell=True,
                                      stdout=subprocess.PIPE,
                                      stderr=subprocess.STDOUT)
            msg = f"shell command run result: {result.stdout.read().decode('utf-8')}"
            logging.exception(msg)
        elif state == "off":
            result = subprocess.Popen(self.on, shell=True,
                                      stdout=subprocess.PIPE,
